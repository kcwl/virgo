...
namespace aquarius
{
	...
	class basic_flex_buffer
	{
		...
		using this_type = basic_flex_buffer<elem_type, traits_type, allocator_type>;
	...
	public:
		basic_flex_buffer(const std::size_t capa = capacity)
			: buffer_(capa, 0)
			, pptr_(0)
			, gptr_(0)
			, pcount_(capa)
		...
		basic_flex_buffer(basic_flex_buffer&& other)
		...
		basic_flex_buffer(const basic_flex_buffer&) = default;
		basic_flex_buffer& operator=(const basic_flex_buffer&) = default;

		basic_flex_buffer& operator=(basic_flex_buffer&& other) noexcept
		{
		...
		virtual ~basic_flex_buffer() = default;
	...
	public:
		...
		bool save(const value_type* data, const size_type size)
		{
			...
			{
				size_type new_capacity = buffer_.size();
